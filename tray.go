...
func trayStringsFor(lang string) trayText {
	...
	if lang == "zh-CN" {
		return trayText{
			openApp: "打开应用页面", openAppTip: "在浏览器中打开 cuterm 应用页面",
			openConfig: "打开配置页面", openConfigTip: "在浏览器中打开 cuterm 配置页面",
			quit: "退出", quitTip: "停止 cuterm 服务并退出",
		}
	}
	return trayText{
		openApp: "Open App", openAppTip: "Open the cuterm app page in your browser",
		openConfig: "Open Settings", openConfigTip: "Open the cuterm settings page in your browser",
		quit: "Quit", quitTip: "Stop the cuterm service and quit",
	}
}
...
func runTray(appURL, configURL func() string, lang string, onQuit func()) {
	systray.Run(func() {
		// The cuterm logo: dark rounded square with a green CU lettermark.
		systray.SetIcon(trayIcon)
		systray.SetTitle("")
		systray.SetTooltip("cuterm")
