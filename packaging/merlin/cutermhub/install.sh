#!/bin/sh
# cuterm-hub 软件中心插件安装脚本（koolshare 1.5 代软件中心 API，
# 适用于梅林改 384/386 armv7l 与梅林改/官改 hnd/axhnd/axhnd.675x 平台）。
...
platform_test(){
	# 1.5 代软件中心（skipd + /koolshare），380 及更早的 1.0 代软件中心没有 skipd
...
install_now(){
	...
	find /koolshare/init.d -name "*cuterm-hub*" | xargs rm -rf >/dev/null 2>&1
	...
	chmod 755 /koolshare/bin/cuterm-hub >/dev/null 2>&1
