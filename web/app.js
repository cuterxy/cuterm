/* cuterm web client.
 *
 * WebSocket protocol: binary frames, first byte is the frame type.
...
(function () {
  ...
  function fetchTerminals() {
    return fetch('/api/terminals').then(function (r) { return r.json(); });
  }
  ...
  function closeTerminal(id) {
    return fetch('/api/terminals/' + id, { method: 'DELETE' });
  }
  ...
  function renderList() {
    ...
    terminals.forEach(function (item) {
      ...
      li.className = 'term-item' + (session && session.id === item.id ? ' active' : '');
      ...
      renameBtn.addEventListener('click', function (ev) {
        ...
        renameTerminal(item.id, newName).then(function () {
          refreshList();
      ...
      closeBtn.addEventListener('click', function (ev) {
        ev.stopPropagation();
        closeTerminal(item.id).then(function () {
          if (session && session.id === item.id) detach();
          refreshList();
        });
      });
      ...
      li.addEventListener('click', function () { attach(item.id); });
      termList.appendChild(li);
    });
  }

  function refreshList() {
    return fetchTerminals().then(function (list) {
      terminals = list;
      renderList();
      // The terminal we are attached to may have exited elsewhere.
      if (session) {
        var mine = null;
        for (var i = 0; i < list.length; i++) {
          if (list[i].id === session.id) { mine = list[i]; break; }
        }
        if (!mine) {
          detach();
        } else {
          document.title = mine.name + (mine.alive ? '' : t('title.exited')) + ' - cuterm';
        }
      }
  ...
  function attach(id) {
    ...
    session = {
      id: id, term: term, fitAddon: fitAddon, ws: null, closedByUs: false,
      onDetach: function () { window.removeEventListener('mouseup', onMouseUp); }
    };

    var proto = location.protocol === 'https:' ? 'wss' : 'ws';
    var ws = new WebSocket(proto + '://' + location.host + '/ws/terminals/' + id);
    ...
    ws.onclose = function () {
      if (session && session.ws === ws && !session.closedByUs) refreshList();
    };
  ...
  newBtn.addEventListener('click', function () {
    createTerminal('').then(function (info) {
      refreshList().then(function () { attach(info.id); });
    }).catch(function (err) { window.alert(err.message); });
  });
