/* cuterm-hub config page: manage the listen port, login launch, the terminal
 * appearance (font, font size, color scheme, scrollback) shared by all
 * attached terminals, and the fleet of cuterm nodes — including each node's
 * shell. Changes apply immediately and are persisted by the hub. */
(function () {
  'use strict';
  ...
  previewTerm.write([
    '',
    '\x1b[1;32m$\x1b[0m echo "\x1b[33mHello, cuterm-hub!\x1b[0m"',
    'Hello, \x1b[1;4mcuterm-hub\x1b[0m — ' + t('cfg.previewSample'),
    '',
  ...
  var cfgNodeList = document.getElementById('cfg-node-list');
  var nodeNameInput = document.getElementById('node-name-input');
  var nodeAddrInput = document.getElementById('node-addr-input');
  ...
  function renderNodeRow(node) {
    ...
    // so the node persists it itself. Only meaningful while the node is up.
    if (node.online) {
      fetch('/api/nodes/' + node.id + '/shells').then(function (r) { return r.json(); }).then(function (cfg) {
        var sel = document.createElement('select');
        sel.className = 'cfg-node-shell';
        sel.title = t('cfg.nodeShellTitle');
        ...
        sel.addEventListener('change', function () {
          post('/api/nodes/' + node.id + '/shell', { shell: sel.value }).catch(function (err) {
            window.alert(t('cfg.nodeShellFail', { error: err.message }));
          });
        });
        li.appendChild(sel);
      }).catch(function () { /* node went away; row refreshes next load */ });
    }
    return li;
  }

  function loadNodes() {
    return fetch('/api/nodes').then(function (r) { return r.json(); }).then(function (list) {
      cfgNodeList.innerHTML = '';
      list.forEach(function (node) {
        cfgNodeList.appendChild(renderNodeRow(node));
      });
    }).catch(function () {
      say(t('cfg.fetchNodesFail'), 'error');
    });
  }

  nodeAddBtn.addEventListener('click', function () {
    var name = nodeNameInput.value.trim();
    var addr = nodeAddrInput.value.trim();
    if (!addr) {
      say(t('cfg.nodeAddrRequired'), 'error');
      return;
    }
    post('/api/nodes', { name: name, addr: addr }).then(function () {
      nodeNameInput.value = '';
      nodeAddrInput.value = '';
      say(t('cfg.nodeAdded'), 'ok');
      loadNodes();
    }).catch(function (err) {
      say(t('cfg.nodeAddFail', { error: err.message }), 'error');
    });
  });
  ...
  form.addEventListener('submit', function (ev) {
    ...
    if (!portChanged && !appearanceChanged && !autostartChanged) {
      say(t('cfg.unchanged'));
      return;
    }
