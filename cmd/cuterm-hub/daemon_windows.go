...
const daemonEnv = "CUTERM_HUB_DAEMON"
...
func daemonize() string {
	...
	fmt.Printf("cuterm-hub is running in the background (pid %d)\n", cmd.Process.Pid)
	fmt.Printf("log file: %s\n", logPath)
	os.Exit(0)
	return "" // unreachable
}

// logFilePath returns the daemon log location (~/.cuterm-hub/cuterm-hub.log),
// falling back to the system temp dir when the home dir is unknown.
func logFilePath() string {
	if home, err := os.UserHomeDir(); err == nil {
		return filepath.Join(home, ".cuterm-hub", "cuterm-hub.log")
	}
	return filepath.Join(os.TempDir(), "cuterm-hub.log")
}
